  ...
  while (1) {
    ...
    if (res == 0) {
      ...
      if (S_ISREG(nextstat.st_mode)) {
        //  process the file by invoking the Handle File
        HandleFile(newfile, doctable, index);
      } else if (S_ISDIR(nextstat.st_mode)) {
        DIR *o_d = opendir(newfile);
        // if can not read the filr directory free it and continue next newfile
        if (o_d == NULL) {
          free(newfile);
          continue;
        }
        // handle directory
        HandleDir(newfile, o_d, doctable, index);
        Verify333(closedir(o_d) == 0);
      }
    }
...
static void HandleFile(char *fpath, DocTable *doctable, MemIndex *index) {
  ...
  // if tab == null, return back to the function
  if (tab == NULL) {
    return;
  }
  ...
  while (NumElementsInHashTable(tab) > 0) {
    ...
    int mia = MIAddPostingList(*index, wp->word, docID, wp->positions);
    Verify333(mia == 1);
